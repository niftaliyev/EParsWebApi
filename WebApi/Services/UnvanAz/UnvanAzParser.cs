                                ...
                                if (response.IsSuccessStatusCode)
                                {
                                    ...
                                    try
                                    {
                                        if (doc.DocumentNode.SelectSingleNode(".//div[@class='breadcrumb']//a[2]").InnerText.Trim() == "Daşınmaz Əmlak")
                                        {
                                            ...
                                            var address = doc.DocumentNode.SelectSingleNode(".//p[@class='infop100 linkteshow']").InnerText.Trim().Split("Ünvan: ")[1];
                                            if (address != null)
                                                announce.address = address;

                                            var text = doc.DocumentNode.SelectSingleNode(".//p[@class='infop100 fullteshow']").InnerText;
                                            if (text != null)
                                                announce.text = text;
                                            ...
                                            var contactName = doc.DocumentNode.SelectSingleNode(".//div[@class='infocontact']//a").InnerText.Trim();
